function draggable_3d(h,varargin)

% Prprak- This is a modified version of the draggable.m code to move
% objects in your figure in 3d. draggable only allows you to move objects
% in 2d. This mostly works with no constraints on the xlim, ylim and zlim.
% The one with constraints has not been tested. So only use 'n'. 
% to call just use draggable_3d(h,'n') where h is the object that you want
% dragged like h= ft_plot_sens(final);
% this function was specifically modified to move the electrodes grids
% around. 
% to get coordinates once you decide on electrode positions, just use
% variable = get(findobj(gca,'Marker','.'))). The final positions can be
% found by running [variable.XData variable.YData variable.ZData]
...
ax_xlim = [-Inf Inf];%get(axh,'XLim');
ax_ylim = [-Inf Inf];%get(axh,'YLim');
ax_zlim = [-Inf Inf];%get(axh,'ZLim');
...
switch lower(constraint)
    case {'n','none'}
        constraint = 'n';
        if isempty(p); p = [ax_xlim ax_ylim ax_zlim]; end;
    case {'h','horizontal'}
        constraint = 'h';
        if isempty(p) 
            p = ax_xlim;
...
function activate_movefcn(axh,obj,eventdata,h)
...
function movefcn(axh,obj,eventdata,h)
% obj here is the *figure* containing the object
...
new_extent = initial_extent + [dpt 0 0 0];
...
xminc = @(dpt) [range(1) - initial_extent(1) dpt(2) dpt(3)];
xmaxc = @(dpt) [range(2) - (initial_extent(1) + initial_extent(4)) dpt(2) dpt(3)];
yminc = @(dpt) [dpt(1) range(3) - initial_extent(2) dpt(3)];
ymaxc = @(dpt) [dpt(1) range(4) - (initial_extent(2) + initial_extent(5)) dpt(3)];
zminc = @(dpt) [dpt(1) dpt(2) range(5) - initial_extent(3)];
zmaxc = @(dpt) [dpt(1) dpt(2) range(6)-(initial_extent(3)+initial_extent(6))];
...
if initial_inrange(5) && ~new_inrange(5)
    % was within, now out of ymax range -- add ymaxc
    corrections = [corrections {zminc}];
end
if initial_inrange(6) && ~new_inrange(6)
    % was within, now out of ymax range -- add ymaxc
    corrections = [corrections {zmaxc}];
end
% if initial_inrange(5) && ~newinrange(5)
...
elseif isfield(props,'XData')
    pos = [props.XData(:)'; props.YData(:)'; props.ZData(:)'];
else
...
if size(pos,1) == 1 % 'Position' property
    ...
    set(obj,'XData',pos(1,:),'YData',pos(2,:),'ZData',pos(3,:));
end
...
elseif isfield(props,'XData')
    ...
    minz = min(props.ZData);
    w = max(props.XData) - minx;
    h = max(props.YData) - miny;    
    l = max(props.ZData) - minz;
    extent = [minx miny minz w h l];
else
...
% extent is in the [x y z w h l] format
% range is in the [xmin xmax ymin ymax zmin zmax] format
% inrange is a 4x1 vector of boolean values corresponding to range limits
inrange = [extent(1) >= range(1) ...
           extent(1) + extent(4) <= range(2) ...
           extent(2) >= range(3) ...
           extent(2) + extent(5) <= range(4)...
           extent(3) >=range(5)...
           extent(3)+extent(6)<=range(6)];
